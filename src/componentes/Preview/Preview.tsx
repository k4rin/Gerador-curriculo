...
export function Preview({ personalData, education, experiences, skills }: PreviewProps) {
  return (
    ...
    className="p-6 overflow-auto h-full bg-[#ffffff]"
    style={{ width: "210mm", minHeight: "297mm" }}>
      {/* Dados Pessoais */}
      <section className="mb-8">
        <h1 className="text-3xl font-bold mb-2">{personalData.name || "Seu Nome"}</h1>
        <p className="text-[#374151] mb-1">{personalData.summary || "Resumo profissional..."}</p>
       ...
       {/* Educação */}
        <section>
          <h2 className="text-2xl font-semibold border-b text-[#374151] pb-1 mb-4"> Formação

          </h2>
          {education.length === 0 ? (
          <h2 className="text-[#6b7280] italic"> Nenhuma formação adicionada </h2>
              ) : 
              (
                education.map((edu) => (
                <div key={edu.id} className="mb-4 ">
                <p className="text-xl font-semibold"> {edu.institution || "Instituição"}</p>
                <p className="text-[#374151] font-medium" > {edu.course || "Curso"}</p>
                <p  className="text-[#4b5563] italic text-sm mb-2">
       ...
       {/* Experiências */}
      <section >
        <h2 className="text-2xl font-semibold border-b text-[#374151] pb-1 mb-4">
          Experiências Profissionais
        </h2>
        {experiences.length === 0 ? (
          <p className="text-[#6b7280] italic">Nenhuma experiência adicionada.</p>
        ) : (
          experiences.map((exp) => (
            <div key={exp.id} className="mb-6">
              <h3 className="text-xl font-semibold">{exp.position || "Cargo"}</h3>
              <p className="text-[#374151] font-medium">{exp.company || "Empresa"}</p>
              <p className="text-[#4b5563] italic text-sm mb-2">
      ...
      <section>
        ...
        {skills.length === 0 ? (
          <p className="text-[#6b7280] italic">Nenhuma habilidade adicionada.</p>
        ) : (
